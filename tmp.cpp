...
void test_int_vector() {
	...
	const std::vector<int> const_vec(5, 101);
	...
	const std::vector<int> e(d);

	std::vector<int>::iterator itb = d.begin();
	std::cout << "random access iteraror (5th element): " << *itb + 4 << std::endl;
	d.erase(itb + 4);
	d.erase(itb + 5, itb + 7);
	std::vector<int>::iterator ite = d.end();
	for (; itb != ite; ++itb) {
		std::cout << *itb << " ";
	}
	std::cout << std::endl;

	std::vector<int>::const_iterator itcb  = e.begin();
	// std::cout << *ictb + 4 << std::endl;
	std::vector<int>::const_iterator itce = e.end();
	for (; itcb != itce; ++itcb) {
		std::cout << *itcb << " ";
	}
	std::cout << std::endl;

	std::vector<int>::reverse_iterator itrb = d.rbegin();
	std::vector<int>::reverse_iterator itre = d.rend();
	for (; itrb != itre; ++itrb) {
		std::cout << *itrb << " ";
	}
	std::cout << std::endl;

	std::vector<int>::const_reverse_iterator itcrb = e.rbegin();
	std::vector<int>::const_reverse_iterator itcre = e.rend();
	...
	std::vector<int> x (6,400);
	std::vector<int> y (7,200);
...
void test_string_vector() {
	...
	std::vector<std::string> s2 (4, "test");
	std::vector<std::string> s3 (s2.begin(), s2.end());
	std::vector<std::string> s4 (s3);
...
void test_int_stack() {
	...
	std::stack<int> ftStack2;
	ftStack2.push(21);
	ftStack2.push(101);
	ftStack2.push(-555);

	std::stack<int> ftStack3;
	ftStack3.push(21);
	ftStack3.push(101);
	ftStack3.push(555);
...
void test_string_stack() {
	...
	std::stack<std::string, std::list<std::string> > lStack;
	if (lStack.empty())
		std::cout << "lStack is empty\n";
	lStack.push("Mary");
	lStack.push("Kate");
	...
	std::stack<std::string> ftStack2;
	ftStack2.push("abc");
	ftStack2.push("hello");
	ftStack2.push("world!");

	std::stack<std::string> ftStack3;
	ftStack3.push("abc");
	ftStack3.push("hello");
	ftStack3.push("world");
...
void test_int_map() {
	...
	std::map<int, int> m5;
	for (size_t i = 0; i < 10; ++i)
		m5.insert(std::make_pair(i, i * 10));
	print_map(m5);

	std::cout << m5.insert(std::make_pair(0, 100)).second << std::endl;
	m2.clear();
	if (m2.empty())
	...
	std::cout << m2.value_comp()(std::make_pair(1, 1), std::make_pair(1, 2)) << std::endl;

	std::cout << "OPERATIONS" << std::endl;
	std::cout << "find" << std::endl;

	std::map<int, int>::iterator fnd = m2.find(0);
	if (fnd == m2.end())
		std::cout << "Element not found" << std::endl;
	std::map<int, int>::iterator fnd2 = m2.find(3);
	if (fnd2 != m2.end())
